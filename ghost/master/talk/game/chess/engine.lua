...
return {
    ...
    content = function(shiori, ref)
      ...
      if type(engine) ~= "table" then
        str:append(SS():p(0))
            ...
            :append(SS():_w(1000):raise("OnChessGameMenu"))
        return str:tostring()
      end
      ...
      for k, v in pairs(option) do
        ...
        str:append(k):append(SS():_l(160)):append("\\_?"):append(v.value):append("\\_?")
        if v.type == "spin" then
          str:append(SS():_l(240):q("【変更】", "OnChessEngineChangeOption", k, v.value))
        elseif v.type == "combo" or v.type == "check" then
          str:append(SS():_l(240):q("【変更】", "OnChessEngineOption", k, v.value))
      ...
      str:append("\\n")
      str:append("\\![*]")
      str:append(SS():q("戻る", "OnChessGameMenu"))
      str:append(SS():_q(false))
      return str:tostring()
    end,
  },
  {
    id  = "OnChessEngineChangeOption",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local name      = ref[0]
      ...
      str:append(SS():inputbox("OnChessEngineChangingOption", 0, value))
      return str:tostring()
    end,
  },
  {
    id  = "OnChessEngineChangingOption",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local name  = __("_ChangingOptionName")
      local value     = ref[0]
      str:append(SS():raise("OnChessEngineOption", name, value))
      return str:tostring()
    end,
  },
  {
    passthrough = true,
    id  = "OnManageChessEngine",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local list      = __("ChessEngineList") or {}
      ...
      local selected  = __("SelectedChessEngine") or ""
      for k, v in pairs(list) do
        local engine  = v
        str:append(SS():q(k, "OnManageChessEngine", engine.name))
        if k == selected then
          str:append(SS():_l(160)):append("【選択中】")
        end
        str:append(SS():_l(220):q("【削除】", "OnChessEngineDelete", engine.name))
        str:append("\\n")
      end
      str:append("\\n")
      str:append("【")
          :append(SS():q("追加", "OnChessEngineAdd"))
          :append("】\\n")
      str:append("\\n")
      str:append("\\![*]")
      str:append(SS():q("戻る", "OnChessGameMenu"))
  ...
  },
  {
    ...
    content = function(shiori, ref)
      ...
      if ref[0] == "open" then
        local path  = ref[2]
        str:append(SS():C():raise("OnChessEngineSpawn", path))
      elseif ref[0] == "cancel" then
        str:append(SS():p(0)):append("キャンセルしたよ")
        str:append(SS():_w(1000):raise("OnManageChessEngine"))
      elseif ref[0] then
        -- TODO error
  ...
  {
    id  = "OnChessEngineDelete",
    content = function(shiori, ref)
      local __        = shiori.var
      local engine_name = ref[0]
      local list  = __("ChessEngineList")
      local selected  = __("SelectedChessEngine")
      list[engine_name] = nil
      if selected == engine_name then
        __("SelectedChessEngine", nil)
      end
      return SS():raise("OnManageChessEngine"):tostring()
    end,
  },
  {
    id  = "OnChessEngineSpawn",
    content = function(shiori, ref)
      local __      = shiori.var
      local str     = StringBuffer()
      ...
      local ret = process("spawn", path, nil, true, "OnChessEngineChildProcess")
      __("_EnginePID", ret())

      if tonumber(ret()) >= 0 then
        process("send", __("_EnginePID"), UCI.tostring{command = UCI.Command.UCI})
        __("_NewEngine", engine)
        local ret = createTimer(shiori,
            10000, function(data)
              if data.command == UCI.Command.INFO then
                if data.str then
                  print(table.concat(data.str, " "))
                end
              elseif data.command == UCI.Command.ID then
                if data.name then
                  engine.name = data.name
                elseif data.author then
                  engine.author = data.author
                end
              elseif data.command == UCI.Command.OPTION then
                assert(data.name)
                local s = nil
              ...
              elseif data.command == UCI.Command.UCIOK then
                return true, SS():raise("OnRegisterChessEngine")
              end
              return nil
            end)
  ...
  },
  {
    ...
    content = function(shiori, ref)
      ...
      str:append(SS():_w(1000):raise("OnChessGameMenu"))
      return str:tostring()
    end,
  },
  {
    id  = "OnQuitChessEngine",
    content = function(shiori, ref)
      local __  = shiori.var
      local str = StringBuffer()
      local process = shiori:saori("process")
      process("send", __("_EnginePID"), UCI.tostring({command = UCI.Command.QUIT}))
      shiori:talk("OnDespawnChessEngine")
    end,
  },
  {
  ...
  },
  {
    ...
    content = function(shiori, ref)
      ...
      if engine == nil then
        ...
        str:append(SS():raise("OnChessGameMenu"))
        return str:tostring()
      end
      local process = shiori:saori("process")
      process("uniqueid", __("_uniqueid"))
      local ret = process("spawn", engine.command, nil, true, "OnChessEngineChildProcess")
      __("_EnginePID", ret())

      if tonumber(ret()) >= 0 then
        process("send", __("_EnginePID"), UCI.tostring({command = UCI.Command.UCI}))
        local s         = createTimer(shiori, 10000, function(data)
          if data.command == UCI.Command.UCIOK then
            return true, SS():raise("OnChessEngineReady")
          end
        end)
        str:append(s)
      else
        str:append("起動に失敗したよ"):append(SS():raise("OnChessGameMenu"))
      end
      return str:tostring()
    end,
  },
  {
    id  = "OnChessEngineReady",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local list      = assert(__("ChessEngineList"))
      local selected  = assert(__("SelectedChessEngine"))
      local process   = shiori:saori("process")
      local engine    = list[selected]
      local game_option = assert(__("ChessGameOption"))
      process("send", __("_EnginePID"), UCI.tostring({
        command = UCI.Command.SETOPTION,
        name    = "UCI_Ponder",
        value   = false,
      }))
      ...
      process("send", __("_EnginePID"), UCI.tostring({command = UCI.Command.ISREADY}))
      local s         = createTimer(shiori, 10000, function(data)
        if data.command == UCI.Command.READYOK then
          return true, SS():raise("OnChessEngineNewGame")
        end
      end)
      str:append(s)
      return str:tostring()
  ...
  {
    id  = "OnChessEngineNewGame",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local process   = shiori:saori("process")
      process("send", __("_EnginePID"), UCI.tostring({command = UCI.Command.UCINEWGAME}))
      str:append(SS():raise("OnChessGameInit"))
      return str:tostring()
    end,
  },
  {
    id  = "OnChessGameEngineTurnBegin",
    content = function(shiori, ref)
      local __        = shiori.var
      local str       = StringBuffer()
      local process   = shiori:saori("process")
      local player    = ChessPlayer.getInstance()
      local _, init, moves  = player:toUCI()
      local options   = __("ChessGameOption")
      ...
      if ponder ~= nil then
        if moves[#moves] == ponder then
          ponderhit = true
          process("send", __("_EnginePID"), UCI.tostring({
            command   = UCI.Command.PONDERHIT,
          }))
        else
          str:append(createTimer(shiori, 10000, function(data)
            if data.command == UCI.Command.BESTMOVE then
              return true, SS():raise("OnChessGameEngineTurnBegin")
            end
          end))
          process("send", __("_EnginePID"), UCI.tostring({
            command   = UCI.Command.STOP,
          }))
          return str
        end
      end
      if not(ponderhit) then
        ...
        process("send", __("_EnginePID"), UCI.tostring({
          command   = UCI.Command.GO,
          btime = time,
          wtime = time,
          byoyomi = 2000,
        }))
      end
      --local s         = createTimer(shiori, "OnChessGameEngineBestMove", "OnChessGameEngineTimeout", true, 100, 100, function(data)
      local s         = createTimer(shiori, time + 3000, function(data)
        ...
        elseif data.command == UCI.Command.BESTMOVE then
          ...
          for i, v in ipairs(moves.pv) do
            print("pv " .. i .. ": " .. v.score .. ", " .. v.move)
          end
          return true, SS():raise("OnChessGameEngineBestMove")
        end
      end)
      str:append(s)
      str:append(SS():_q(true))
  ...
  },
  {
    ...
    content = function(shiori, ref)
      ...
      local data  = UCI.parse(line)
      if data.command == UCI.Command.NONE then
        -- TODO warning
      end
      if data.command == UCI.Command.ERROR then
        -- TODO error
        print(data.reason)
      else
