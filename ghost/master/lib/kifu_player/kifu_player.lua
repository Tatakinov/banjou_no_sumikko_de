...
local NL        = require("kifu_player.nl")
local Parser    = require("kifu_player.parser")
local Position  = require("kifu_player.position")
...
function M:normalize_all()
  ...
  if initial then
    ...
    data  = USI.parseInit(InitialPreset.toSfen(InitialPreset.HIRATE))
  end
  ...
  if move then
    if move.from and move.relative then
      ...
      if move.from == nil or move.relative == nil then
        ...
        elseif #choice > 1 then
          ...
          if move.relative then
            ...
            if move.piece == CSA.RY or move.piece == CSA.UM then
              if move.relative == Relative.C then
                assert(other)
                if other.sub == Relative.L then
                  move.relative = Relative.R
                elseif other.sub == Relative.R then
                  move.relative = Relative.L
                end
              end
            end
      ...
      if move.from and move.promote == nil then
        local promote, force  = Misc.canPromote(move.color, move.from.y, move.to.y, move.piece)
        assert(force == false)
        if promote then
          move.promote  = false
        end
...
function M:getSashite(num)
  ...
  if move then
    ...
    local str = StringBuffer()
    str:append(Misc.csa2special(move_format.special))
...
function M:generateMoves()
  local moves = {}
  local move_moves  = self.position:moveGenerateMove(self:getTeban())
  local hit_moves   = self.position:moveGenerateHit(self:getTeban())
  local king_moves  = self.position:moveGenerateKing(self:getTeban())
...
function M:toUSI()
  local str       = StringBuffer()
  local init, sep = Parser.USI.toUSIinit(self.jkf.initial)
  local moves     = {}
  ...
  if current > 0 then
    ...
    while self:getTesuu() < current do
      self:forward()
      local move  = Parser.USI.toUSImove(self:getMoveFormat(), self:getTesuu())
      if move then
...
function M:toSfen(reverse)
  ...
  for _, color in ipairs(Color.LIST) do
    for _, kind in ipairs({"HI", "KA", "KI", "GI", "KE", "KY", "FU"}) do
      local color = color
      if reverse then
        color = Color.reverse(color)
      end
